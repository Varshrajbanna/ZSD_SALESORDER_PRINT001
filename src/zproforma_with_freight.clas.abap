CLASS zproforma_with_freight DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZPROFORMA_WITH_FREIGHT IMPLEMENTATION.
...
**********************************************************************
    ...
    lv_xml =
     |<form1>| &&
                  ...
                  |<Table1>| &&
                  |<HeaderRow>| &&
                  |<PriceUSD>{ lv_prece-TransactionCurrency }</PriceUSD>| &&
...
lv_xml = lv_xml &&
            ...
            |<PriceUSDKGS>{ wa_final-conditionrateamount - wa_final-discount DECIMALS = 2 }</PriceUSDKGS>| &&
            |<UOM>{ wa_final-uom }</UOM>| &&
            |<NETPRICE>{ wa_final-netprice DECIMALS = 2 }</NETPRICE>| &&
            |<DIFFERENTIALAIRFREIGHT>{ wa_final-differentialamount DECIMALS = 2 }</DIFFERENTIALAIRFREIGHT>| &&
...
*              |<TOTNETVALUE>{ lv_tot_net }</TOTNETVALUE>| &&
 ...
 |</form1>|.
    ...
    CALL METHOD zadobe_print=>adobe
      EXPORTING
        xml       = lv_xml
        form_name = 'Proforma_ freight_breakup'
      RECEIVING
        result    = result12.
